 ...
 *   - n - 1 cycles on 'subi';
 *   - 2 * (n - 1) cycles on 'brne' (including the branch misprediction
 *     penalty);
 ...
 * instructions: 'movw', 'sbiw', 'brne'. '_delay_loop_2(n)' will then spend CPU
 * cycles like this:
 ...
 * as unsigned.
 * 
 * As soon as 'T' reaches 3 * 256, generated functions ditch '_delay_loop_1(n)'
 * in favor of '_delay_loop_2(n)'; the reason for this is interesting.
 * If we did use '_delay_loop_1(n)' with the value 3 * 256, the argument 'n'
 * would be 0 (the result of casting 256 to an unsigned byte). Unfortunately,
 * based on some experiments, the compiler's output is less consistent in this
 * scenario. Sometimes it still generates the instructions 'mov', 'subi' and
 * 'brne', and it takes exactly 3 * 256 cycles (the first loop run causes an
 * underflow which sets the counter to 255). Other times, it omits the first
 * 'mov' instruction and recycles a register which has just been zeroed, making
 * the delay one CPU cycle shorter.
 */
template<uint16_t T>
